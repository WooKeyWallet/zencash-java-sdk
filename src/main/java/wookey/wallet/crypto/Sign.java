...
import wookey.wallet.utils.Assertions;
...
public class Sign {
     ...
     */
    public static BigInteger publicKeyFromPrivate(BigInteger privKey, Object... argument) {
        ECPoint point = publicPointFromPrivate(privKey);
        byte[] encoded = point.getEncoded(false);
        if (argument != null && argument.length != 0 && (boolean) argument[0]) {
            encoded = point.getEncoded(true);
        }
    ...
    public static byte[] toDER(ECDSASignature ecdsaSignature) {
        ...
        // Pad values
        if ((r[0] & 0x80) == 1) {
            r = Utils.byteConcat(r, new byte[]{0}, r.length + 1);
        }
        // Pad values
        if ((s[0] & 0x80) == 1) {
            s = Utils.byteConcat(s, new byte[]{0}, s.length + 1);
        }
        r = rmPadding(r);
        s = rmPadding(s);

        while (!(s[0] != 0) && !((s[1] & 0x80) == 1)) {
            s = Utils.subarray(s, 1, s.length);
        }

        byte[] arr = new byte[]{0x02};
        arr = constructLength(arr, (byte) r.length);
        arr = Utils.byteConcat(arr, r, arr.length + r.length);
        arr = Utils.byteConcat(arr, new byte[]{0x02}, arr.length + 1);
        arr = constructLength(arr, (byte) s.length);
        byte[] backHalf = Utils.byteConcat(arr, s, arr.length + s.length);
        byte[] res = new byte[]{0x30};
        res = constructLength(res, (byte) backHalf.length);
        res = Utils.byteConcat(res, backHalf, res.length + backHalf.length);

        return res;
    }


    public static byte[] rmPadding(byte[] buf) {
        int i = 0;
        ...
        return Utils.subarray(buf, i, buf.length);
    }

    public static byte[] constructLength(byte[] arr, byte len) {
        if (len < 0x80) {
            arr = Utils.byteConcat(arr, new byte[]{len}, arr.length + 1);
            return arr;
        }
        int octets = 1 + (new Double(Math.log(len) / Math.log(Math.pow(Math.E, 2))).intValue() >>> 3);
        arr = Utils.byteConcat(arr, new byte[]{(byte) (octets | 0x80)}, arr.length + 1);
        while (--octets == 1) {
            arr = Utils.byteConcat(arr, new byte[]{(byte) ((len >>> (octets << 3)) & 0xff)}, arr.length + 1);
        }
        return arr = Utils.byteConcat(arr, new byte[]{len}, arr.length + 1);
    }
}
