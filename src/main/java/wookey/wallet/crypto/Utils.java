...
public class Utils {
    ...
    public static void int32ToByteArrayLE(long val, byte[] out, int offset) {
    ...
    public static long readInt8(byte[] bytes, int offset) {
        if ((bytes[offset] & 0x80) != 1) return bytes[offset];
        return ((0xff - bytes[offset] + 1) * -1);
    }
    ...
    public static byte[] ripemd160Digest(byte[] input) {
        RIPEMD160Digest digest = new RIPEMD160Digest();
    ...
    public static byte[] byteMerger(byte[] bt1, byte[] bt2, int length) {
        byte[] bt3 = new byte[length];
        System.arraycopy(bt1, 0, bt3, 0, bt1.length);
        System.arraycopy(bt2, 0, bt3, bt1.length, 4);
        return bt3;
    }

    public static byte[] byteConcat(byte[] bt1, byte[] bt2, int length) {
        byte[] bt3 = new byte[length];
        System.arraycopy(bt1, 0, bt3, 0, bt1.length);
    ...
    public static int writeUInt8(byte[] buffer, int value, int offset) {
        value = +value;
        offset = offset | 0;
    ...
    public static String getPushDataLength(String hex) {
        int hexLength = HEX.decode(hex).length;
        return Utils.HEX.encode(new VarInt(hexLength).encode());
    }

    public static String numToVarInt(int num) {
        return Utils.HEX.encode(new VarInt(num).encode());
    }
